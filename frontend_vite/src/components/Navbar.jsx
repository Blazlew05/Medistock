import { Activity, LogOut, Menu, ShoppingCart, User, X } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl px-4">
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            <Link to="/catalogo" className="px-3 py-2 text-sm font-medium text-slate-600 hover:text-primary-700">
              Catálogo
            </Link>
            {esCliente && (
              <Link to="/mis-pedidos" className="px-3 py-2 text-sm font-medium text-slate-600 hover:text-primary-700">
                Mis pedidos
              </Link>
            )}
            {rutaInterna && (
              <Link to={rutaInterna} className="px-3 py-2 text-sm font-medium text-primary-700 hover:text-primary-900">
                Panel interno
              </Link>
            )}
          </nav>
          ...
          <div className="flex items-center gap-3">
            <Link to="/carrito" className="relative p-2 text-slate-600 hover:text-primary-700">
            ...
            {usuario ? (
              ...
              <Link to="/login" className="hidden md:flex items-center gap-2 rounded-lg bg-primary-600 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-700">
        ...
        {menuAbierto && (
          <div className="md:hidden border-t border-slate-200 py-3 space-y-2">
            <Link to="/catalogo" className="block px-2 py-2 text-sm font-medium text-slate-700">Catálogo</Link>
            {esCliente && <Link to="/mis-pedidos" className="block px-2 py-2 text-sm">Mis pedidos</Link>}
            {rutaInterna && <Link to={rutaInterna} className="block px-2 py-2 text-sm">Panel interno</Link>}
            {usuario ? (
              ...
              <Link to="/login" className="block px-2 py-2 text-sm font-semibold text-primary-700">Ingresar</Link>
            )}
          </div>
        )}
      </div>
