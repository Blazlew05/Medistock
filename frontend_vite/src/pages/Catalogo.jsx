...
export default function Catalogo() {
  ...
  useEffect(() => {
    api.get("/api/v1/productos/categorias").then(setCategorias).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const path = categoria
      ...
      : "/api/v1/productos";
    api.get(path).then(setProductos).finally(() => setCargando(false));
  }, [categoria]);
...
function CardProducto({ producto }) {
  ...
  return (
    <div className="card flex flex-col">
      <Link to={`/producto/${producto.codigo}`} className="flex-1">
