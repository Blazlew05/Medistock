...
export default function AnalistaPanel() {
  return (
...
function AnalistaVista() {
  ...
  function cargar() {
    setCargando(true);
    api.get("/api/v1/analista/pagos", true).then(setPagos).finally(() => setCargando(false));
  }
  useEffect(() => { cargar(); }, []);

  async function auditar(pagoId, estado) {
    if (!window.confirm(`¿Marcar pago #${pagoId} como ${estado}?`)) return;
