import { Building2, CheckCircle2, Search, Warehouse } from "lucide-react";
...
function EjecutivoLayout() {
  const [tab, setTab] = useState("aprobar");
...
function PendientesAprobacion() {
  ...
  function cargar() {
    setCargando(true);
    api.get("/api/v1/ejecutivo/ordenes/pendientes", true).then(setOrdenes).finally(() => setCargando(false));
  }
  useEffect(() => { cargar(); }, []);

  async function aprobar(id) {
    if (!window.confirm("¿Aprobar esta orden? El cliente podrá proceder al pago.")) return;
...
function StockMultiBodega() {
  ...
  async function buscar(e) {
    ...
    setError("");
    try {
      const res = await api.get(`/api/v1/ejecutivo/productos/${productoId}/stock`, true);
      setDatos(res);
    } catch (err) {
