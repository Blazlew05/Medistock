...
import { formatCLP, getAuth, type ItemCarrito } from "@/lib/api";
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center">
        ...
        <Link href="/catalogo" className="btn-primary mt-6">Ver catálogo</Link>
      </div>
    );
  }
          ...
          {items.map((item) => (
            <div key={item.producto.codigo} className="card flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <Link href={`/producto/${item.producto.codigo}`} className="font-semibold text-ink line-clamp-1 hover:text-primary-700">
