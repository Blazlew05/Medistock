...
import { api, saveAuth, type Usuario } from "@/lib/api";
...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(RUTAS[res.usuario.rol] || "/");
    } catch (err) {
      ...
      <div>
        ...
        <form onSubmit={handleLogin} className="card space-y-4">
          ...
          <div className="text-center text-sm text-slate-600">
            ¿No tienes cuenta? <Link href="/registro" className="font-semibold text-primary-700 hover:underline">Regístrate</Link>
          </div>
        </form>
      </div>
