...
import { api, saveAuth, type Usuario, type RolUsuario } from "@/lib/api";

export default function RegistroPage() {
  const router = useRouter();
  const [rol, setRol] = useState<RolUsuario>("cliente_paciente");
  ...
  async function handleRegistro(e: React.FormEvent) {
    ...
    setCargando(true);
    try {
      await api.post<Usuario>("/api/v1/auth/registro", { ...form, rol });
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12">
      ...
      <div className="flex gap-2 mb-6">
        {(["cliente_paciente", "cliente_institucion"] as const).map((r) => (
          <button
            key={r}
            type="button"
            onClick={() => setRol(r)}
      ...
      <form onSubmit={handleRegistro} className="card space-y-4">
        ...
        <div className="text-center text-sm text-slate-600">
          ¿Ya tienes cuenta? <Link href="/login" className="font-semibold text-primary-700 hover:underline">Inicia sesión</Link>
        </div>
      </form>
    </div>
  );
