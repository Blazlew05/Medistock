  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <div className="flex gap-2 border-b border-slate-200 mb-6 overflow-x-auto">
        {(["dashboard", "productos", "ordenes"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
...
function DashboardTab() {
  ...
  useEffect(() => {
    api.get<Dashboard>("/api/v1/admin/dashboard", true).then(setData).catch(() => {});
  }, []);
    ...
    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {cards.map((c) => (
        <div key={c.label} className="card">
          <div className={`flex h-10 w-10 items-center justify-center rounded-lg bg-${c.color}-50 text-${c.color}-600 mb-3`}>
...
function ProductosTab() {
  ...
  function cargar() {
    api.get<ProductoAdmin[]>("/api/v1/admin/productos", true).then(setProductos).catch(() => {});
  }
...
function OrdenesTab() {
  ...
  useEffect(() => {
    api.get<Orden[]>("/api/v1/admin/ordenes", true).then(setOrdenes).catch(() => {});
  }, []);
